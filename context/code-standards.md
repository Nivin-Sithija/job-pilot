...
- **OAuth cookies**: the PKCE code-verifier cookie in `actions/auth.ts` sets `secure` from `NEXT_PUBLIC_APP_URL`'s protocol (`IS_HTTPS_APP_URL`), never hardcode `secure: true` — a `Secure` cookie is silently dropped by the browser on a plain-HTTP origin, which breaks the OAuth round trip with no client-visible error beyond a generic exchange failure. The InsForge SDK's *own* session cookies (`accessToken`/`refreshToken`, set during `exchangeOAuthCode`) have the same constraint but stamp `Secure` purely from `NODE_ENV === "production"` (not configurable) — so production **must** be served over HTTPS or the post-login redirect silently bounces back to `/login` with no `?error=`. This is why the droplet runs HTTPS via a `nip.io` hostname + Caddy/Let's Encrypt (see architecture.md's Hosting section), not plain HTTP on the bare IP.
