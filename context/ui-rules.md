...
- Never use `position: fixed` for UI elements — use normal flow layout. **Exception:** the native HTML `<dialog>` element (via `.showModal()`) is the approved way to center an overlay/popup regardless of scroll position — the browser's own top-layer handles the fixed-style centering and backdrop, not CSS we author, so this isn't a violation of the rule. See `ActionResultDialog` in `ui-registry.md` for the established pattern. Never hand-roll a centered overlay with our own `position: fixed`/`position: absolute` — use `<dialog>` instead.
- **Always add `m-auto` explicitly to any `<dialog>` element.** Tailwind v4's Preflight (`@layer base { *, :after, :before, ::backdrop { margin: 0; ... } }`) is an author-origin stylesheet rule, and author styles always beat the browser's user-agent stylesheet regardless of selector specificity — so it silently overrides the UA stylesheet's `dialog:modal { margin: auto }` centering rule, leaving the dialog pinned to the top-left of its `inset: 0` box instead of centered. Confirmed by inspecting the actual compiled Tailwind output, not just suspected. `m-auto` as an explicit author rule on the dialog itself restores centering.
