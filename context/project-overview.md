...
JobPilot is a full stack AI-powered job hunting assistant. The user sets up their profile once, uploads their resume, and the agent automatically discovers relevant jobs from ITPro.lk — scoring each one against the user's profile using Gemini 2.5 Flash. For jobs they're interested in, the agent researches the company across their public web pages and builds a structured dossier — company overview, tech stack, culture, why the role exists, and interview prep. The user reviews everything and applies with one click.
...
### Finding Jobs — ITPro.lk Discovery

- User goes to Find Jobs page
- Enters job title and location
- Clicks Find Jobs button
- Agent fetches a recent batch of listings from the ITPro.lk API and filters them against the user's search criteria client-side
...
- Full paginated job list below:
  ...
  - "Jobs by ITPro.lk" credit displayed on job listings
...
- ITPro.lk API job discovery — fetches recent IT/tech job listings, filtered client-side by title and location keywords
...
- "Jobs by ITPro.lk" credit on all job listings
...
- ITPro.lk job discovery returns relevant tech jobs for any title and location search
