...
- Result (success or failure) reports through the shared `ActionResultDialog` (`components/shared/ActionResultDialog.tsx`, introduced in Feature 07) — same component Save Profile and Extract from Resume already use. Do not add inline status text or a separate dialog for this button.
...
### 10 ITPro.lk Job Discovery

Agent calls the ITPro.lk public API to find Sri Lankan tech jobs, scores them against user profile, saves to DB.

**Logic:**

- POST /api/agent/find receives jobTitle and location from client
- Call ITPro.lk API:
  - GET https://itpro.lk/api/v1/jobs?limit=100
  - No API key required for reads — public endpoint, no signup
...
- Back to Jobs link
- Job header — company logo placeholder, job title, company name, match score badge with percentage, View Job Post button (links to source_url — the ITPro.lk listing page)
...
- Job Description section — description content from ITPro.lk
...
- Apply Now button (links to external_apply_url, opens in new tab)
...
- Derive company homepage URL — ITPro.lk's `website` field on the job record is usually the company's actual homepage already (no redirect-following needed, unlike Adzuna):
